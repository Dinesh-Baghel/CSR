...
namespace Infrastructure.Services.V1
{
    public class FiltersService : IFilters
    {
        ...
        public async Task<GetDisplayFiltersRes> GetDisplayFilters(GetDisplayFiltersReq req)
        {
            GetDisplayFiltersRes res = new();
            var result = await _dapperHelper.QueryFirstOrDefaultAsync<dynamic>("PRO_GET_BSR_FILTER_VALUES", new { userId=req.userId, WhereClause=req.WhereClause, RequiredColNames=req.RequiredColNames, SEASON = req.SEASON });
