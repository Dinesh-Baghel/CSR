...
namespace Infrastructure.Services.V1
{
    public class MenuService : IMenu
    {
        ...
        public async Task<List<Menu>> GetAllMenus(SelectListReq req)
        {
            ...
            var ResSet = await _dapperHelper.ExecuteStoredProcedureMultipleListAsync<Menu, RoleList>("Select_SelectAll_SelectList", parms);
            if (ResSet.Item1.ToList().Count > 0)
            {
                ...
                {
                    res = ResSet.Item1.ToList();
                }
            }
            return res;
        }
        public async Task<List<MenusList>> GetMenuList(SelectListReq req)
        {
            var res = new List<MenusList>();
            ...
            var result = await _dapperHelper.ExecuteStoredProcedureListAsync<MenusList>("Select_SelectAll_SelectList", parameters);
        ...
        public async Task<InUpRes> SetMenu(Menu req)
        {
            ...
            var ResSet= await _dapperHelper.ExecuteStoredProcedureAsync("Insert_Update_MenuMaster", parameters);
