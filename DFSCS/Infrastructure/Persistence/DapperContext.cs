...
namespace Infrastructure.Persistence
{
    public class DapperContext : IDapper
    {
        ...
        public DapperContext(IConfiguration configuration, New_Enc_Dec Enc_Dec)
        {
            _Enc_Dec = Enc_Dec;
        ...
        public async Task<IEnumerable<T>> QueryWithMappingAsync<T>(string storedProc, object? param = null)
        {
            // Register the column-to-property mapping for this type T
        ...
        public async Task<Dictionary<string, string>> ExecuteStoredProcedureAsync(
     string storedProc,
     object inputParam,
     Dictionary<string, DbType> outputParams)
        {
            using (var connection = GetConnection())
            {
                ...
                foreach (var kvp in outputParams)
                {
                    var value = parameters.Get<object>(kvp.Key);
                    result[kvp.Key] = value?.ToString() ?? string.Empty;
                }
